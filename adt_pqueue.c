...
#include "functions.h"   // Inclusione dell'header file per l'aggiunta ai files delle attività.
...
void ottieni_queue_da_file(pqueue p, FILE* fp){
     ...
     while(fgets(temp, sizeof(temp), fp) != NULL){          //Se la riga non è vuota esce dal ciclo
          ...
          fgets(temp, sizeof(temp), fp);                    //Stesse operazioni ma con la seconda riga
          strncpy(corso, temp, 16);
          corso[15] = '\0';
          corso[strcspn(corso, "\n")] = '\0';

          fgets(temp, sizeof(temp), fp);                    //Legge terza riga
          ...
          if(sscanf(temp, "%10s %8s %8s %d %d %d", data_scadenza, tempo_stimato, tempo_passato, &priorita, &id, &stato) == 6) {
               ...
               //Usa i campi presi dal file report, li passa in input a inserisci_item() per generarne un item e lo inserisce nella queue con inserimento_coda()
...
void scrivi_file_report_queue(pqueue p){
     ...
     FILE* fp = fopen("files_txt/report_settimanale.txt", "w");
...
int controlla_stato_attivita(pqueue q, int id){
     ...
     int z = stato_completamento_item(q->vet[id]);
     
     //Se attività completata o in ritardo, restituisce z = -1 oppure z = 0
     if((z == -1)||(z == 0)) return z;
